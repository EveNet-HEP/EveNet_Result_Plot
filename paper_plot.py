...
DEFAULT_QE_CONFIG = {
    ...
    "poi": {
        ...
        "left_indicator_text": r"Separable state ($\mathcal{C} = 0$)",
        "right_indicator_x": 0.3172751210597295,
        "right_indicator_color": "gray",
        "right_indicator_linestyle": "--",
        "show_row_separators": True,
        "right_x_min_factor": 0.995,
        "right_x_max_factor": 1.03,
...
DEFAULT_BSM_CONFIG = {
    ...
    "systematics": {
        "sic": {
            ...
            "x_label": r"($\mathrm{SIC}-\langle \mathrm{SIC} \rangle)/\sigma_\mathrm{SIC}$",
            # "cmap": "coolwarm",
            "cmap": LinearSegmentedColormap.from_list(
                "custom_jes",
                ["#0072B2", "#E5ECF6", "#D55E00", ],  # low → mid → high
...
DEFAULT_AD_CONFIG = {
    ...
    "mass_distribution": {
        "enabled": True,
        "fpr_thresholds": [-1.0, 1.0, 0.3, 0.1],
        "show_l_reweighted_upper": False,
        "cms_label": "2016 CMS Open Data DoubleMuon",
        "show_cms_label": False,
        "cms_label_size": 16.0,
        "cms_label_weight": "bold",
        "cms_label_x": 0.01,
        "cms_label_y": 1.01,
        "cme_lumi_label": r"$\sqrt{s}=13\ \mathrm{TeV},\ 8.7\ \mathrm{fb}^{-1}$",
        "show_cme_lumi_label": True,
        "cme_lumi_size": 16.0,
        "cme_lumi_x": 0.99,
        "cme_lumi_y": 1.01,
        ...
        "sideband_text_x_offset": 0.025,
        "sideband_text_size": 18.0,
        "sideband_text_weight": "bold",
        "sideband_text_style": "normal",
        "lower_panel": {
            "enabled": True,
            "fpr_thresholds": [-1.0],
            "height_ratio": (2, 1),
        ...
        "plot_files": [
            {
                "path": "bump_hunting_12bins_quintic_fit_seed192041.npz",
                "f_name": "ad_mass_distribution",
                "caption": "AD: mass distribution (12-bin quintic fit)",
                "sideband_text": r"OS$\rightarrow$OS",
            },
            {
                "path": "bump_hunting_12bins_quintic_fit_no_signal_seed192041.npz",
    ...
    if mass_cfg.get("enabled", False):
        ...
        for idx, file_cfg_raw in enumerate(plot_files):
            ...
            mass_result = plot_ad_mass_distribution(
                ...
                show_cms_label=file_cfg.get("show_cms_label", mass_cfg.get("show_cms_label", True)),
                ...
                show_cme_lumi_label=file_cfg.get(
                    "show_cme_lumi_label",
                    mass_cfg.get("show_cme_lumi_label", True),
                ),
                ...
                sideband_text=file_cfg.get("sideband_text", mass_cfg.get("sideband_text")),
                sideband_text_y=file_cfg.get("sideband_text_y", mass_cfg.get("sideband_text_y", 0.84)),
                sideband_text_x_offset=file_cfg.get(
                    "sideband_text_x_offset",
                    mass_cfg.get("sideband_text_x_offset", 0.015),
                ),
                sideband_text_size=file_cfg.get("sideband_text_size", mass_cfg.get("sideband_text_size")),
                sideband_text_weight=file_cfg.get(
                    "sideband_text_weight",
                    mass_cfg.get("sideband_text_weight", "normal"),
                ),
                sideband_text_style=file_cfg.get(
                    "sideband_text_style",
                    mass_cfg.get("sideband_text_style", "normal"),
                ),
                ...
                show_l_reweighted_upper=file_cfg.get(
                    "show_l_reweighted_upper",
                    mass_cfg.get("show_l_reweighted_upper", True),
                ),
